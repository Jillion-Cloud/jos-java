...
class JOSClientTest {
    ...
    private static JOSClient JOSClient;

    @BeforeAll
    public static void beforeAll() {
        JOSClient = new JOSClientBuilder()
    ...
    void test1_listBuckets() {
        ...
        qry.setPageSize(20);
        PageResponse<BucketCO> response = JOSClient.listBuckets(qry);
    ...
    void test2_createBucket() {
        BucketCreateReq req = BucketCreateReq.builder()
                ...
                .quotaSize(100)
                .build();
        Response response = JOSClient.createBucket(req);
        Assertions.assertTrue(response.isSuccess());
        log.info("{}", JSON.toJSON(response));
    }
    ...
    void test_listObjects() {
        ObjectListQry qry = ObjectListQry.builder()
                .bucketName("test")
                .build();
        PageResponse<ObjectInfoListCO> response = JOSClient.listObjects(qry);
        Assertions.assertTrue(response.isSuccess());
        log.info("{}", JSON.toJSON(response));
    }
    ...
    void test_putObject() {
        ObjectPutReq req = ObjectPutReq.builder()
                ...
                .build();
        Response response = JOSClient.putObject(req, new File("C:\\Users\\TDH\\Downloads\\content-type.txt"), null);
        log.info("{}", JSON.toJSON(response));
        Assertions.assertTrue(response.isSuccess());
    }
    ...
    void test_getObject() {
        ObjectGetQry qry = ObjectGetQry.builder()
                ...
                .objectPath("/1228/content-type2.txt")
                .build();
        SingleResponse<ObjectInfoCO> response = JOSClient.getObject(qry);
        log.info("{}", JSON.toJSON(response));
        Assertions.assertTrue(response.isSuccess());
    }
    ...
    void test_getObjectContent() throws Exception {
        ObjectGetQry qry = ObjectGetQry.builder()
                ...
                .objectPath("/1228/content-type2.txt")
                .build();
        ObjectContentCO contentCO = JOSClient.getObjectContent(qry);
    ...
    void test_getPresignedUrl() {
        PresignedUrlGetQry qry = PresignedUrlGetQry.builder()
                ...
                .method("GET")
                .build();
        SingleResponse<PresignedUrlCO> response = JOSClient.getPresignedUrl(qry);
        log.info("{}", JSON.toJSON(response));
        Assertions.assertTrue(response.isSuccess());
    }
}