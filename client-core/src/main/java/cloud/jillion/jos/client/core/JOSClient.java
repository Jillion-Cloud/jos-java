...
import cloud.jillion.jos.client.core.api.BucketClient;
import cloud.jillion.jos.client.core.api.ObjectClient;
import cloud.jillion.jos.client.core.model.request.*;
import cloud.jillion.jos.client.core.model.response.*;
import cloud.jillion.jos.client.core.rest.BucketRestClient;
import cloud.jillion.jos.client.core.rest.ObjectRestClient;
import cloud.jillion.jos.client.core.utils.HttpUtils;
 ...
 * @since 1.0.0
 */
public class JOSClient implements BucketClient, ObjectClient {
    ...
    JOSClient(JOSClientBuilder builder) {
    ...
    private void validateConf(JOSClientBuilder builder) {
        if (StringUtils.isBlank(builder.getServerUrl())) {
            throw new JOSClientException("FOS server url must be not empty");
        }
        if (StringUtils.isBlank(builder.getAccessKey())) {
            throw new JOSClientException("FOS access key must be not empty");
        }
        if (StringUtils.isBlank(builder.getAccessSecret())) {
            throw new JOSClientException("FOS access secret must be not empty");
        }
        // validate server url
        boolean isValidUrl = HttpUtils.isValidUrl(builder.getServerUrl());
        if (!isValidUrl) {
            throw new JOSClientException("FOS serverUrl is invalid");
        }
    }
